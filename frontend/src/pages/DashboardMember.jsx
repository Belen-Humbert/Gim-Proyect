...
const DashboardMember = () => {
  ...
  // Para plan libre/3 dias - agregar entrenamiento
  ...
  // Para plan personalizado - check de días
  ...
  console.log('Plan:', plan?.plan?.name, 'isPersonalized:', isPersonalized)

  // ── Check día de rutina ──
  const handleCheckDay = async (day) => {
    const log = routine?.days?.find(d => d.id === day.id)
    ...
    try {
      if (isCompleted) {
        ...
        await api.post(`/routines/day/${day.id}/complete`, {
  ...
  // ── Agregar ejercicio al workout form ──
  const addExerciseToForm = () => {
    setWorkoutForm({
      ...workoutForm,
      exercises: [...workoutForm.exercises, { name: '', sets: '', reps: '', weight: '', notes: '' }]
    })
  }
  ...
  // ── Guardar entrenamiento libre ──
  const handleSaveWorkout = async () => {
    setSavingWorkout(true)
    try {
      await api.post('/workout-logs/free', {
  ...
  return (
    <div className="mb-root">
            ...
            {!plan ? (
              ...
              <div className="mb-plan-grid">
                ...
                <div className={`mb-info-card ${daysLeft !== null && daysLeft <= 7 ? 'warning' : ''} ${daysLeft !== null && daysLeft < 0 ? 'expired' : ''}`}>
                  ...
                  {daysLeft !== null && daysLeft >= 0 && (
                    <div className="mb-info-note">
                      {daysLeft === 0 ? 'Vence hoy' : `Vence en ${daysLeft} días`}
                    </div>
                  )}
                  {daysLeft !== null && daysLeft < 0 && (
                    <div className="mb-info-note expired">Venció hace {Math.abs(daysLeft)} días</div>
                  )}
                </div>
            ...
            <div className="mb-header">
              <div>
                ...
                <p className="mb-subtitle">
                  {isPersonalized
                    ? 'Rutina personalizada — marcá los días completados'
                    : 'Registrá tus entrenamientos'}
                </p>
              </div>
              {!isPersonalized && (
                <button className="mb-btn-primary" onClick={() => setShowAddWorkout(true)}>
                      ...
                      return (
                        <div key={day.id} className={`mb-day-card ${isCompleted ? 'completed' : ''}`}>
                          <div className="mb-day-header">
                            ...
                            <button
                              className={`mb-check-btn ${isCompleted ? 'checked' : ''}`}
                              onClick={() => handleCheckDay(day)}
                              disabled={checkingDay === day.id}
                            >
                              {checkingDay === day.id ? '...' : isCompleted ? '✅ Completado' : '⬜ Marcar como hecho'}
                            </button>
                          </div>
                          {isCompleted && completedAt && (
                            <div className="mb-completed-date">
            ...
            {isPersonalized ? (
              <div className="mb-history-list">
                {routine?.days?.filter(d => d.workoutLogs?.length > 0).length === 0 ? (
                  <div className="mb-empty-plan">
                    <div className="mb-empty-icon">📅</div>
                    <h3>No hay días completados todavía</h3>
                  </div>
                ) : (
      ...
      {/* ── Modal agregar entrenamiento libre ── */}
      {showAddWorkout && (
        <div className="mb-overlay" onClick={() => setShowAddWorkout(false)}>
          <div className="mb-modal" onClick={e => e.stopPropagation()}>
            <div className="mb-modal-header">
              <h2>Registrar Entrenamiento</h2>
              <button className="mb-modal-close" onClick={() => setShowAddWorkout(false)}>✕</button>
            </div>
            ...
            <div className="mb-modal-footer">
              <button className="mb-btn-secondary" onClick={() => setShowAddWorkout(false)}>Cancelar</button>
